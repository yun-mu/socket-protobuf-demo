...
func (this *RedisDBCntlr) GETINT(key string) (int, error) {
	return redis.Int(this.conn.Do("GET", key))
}
...
func (this *RedisDBCntlr) LRANGEGetBytes(key string, start, end int) ([][]byte, error) {
	return redis.ByteSlices(this.conn.Do("LRANGE", key, start, end))
}
